...
func main() {
	...
	distFromCenter := 10000000

	for _, plotA := range routeA.history {
		for _, plotB := range routeB.history {
			if reflect.DeepEqual( plotA, plotB ) {
				tmp := abs(plotA.x) + abs(plotA.y)

				if tmp < distFromCenter {
					distFromCenter = tmp 
				}
			}
		}
